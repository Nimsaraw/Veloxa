...
if (isset($_GET["id"])) {
    ...
    product.brand_has_model_id,product.condition_condition_id,product.status_status_id,product.users_email,product.color_color_id,
    model.model_name AS mname,brand.brand_name AS bname FROM `product` INNER JOIN `brand_has_model` ON 
    brand_has_model.model_model_id=product.brand_has_model_id INNER JOIN `brand` ON brand.brand_id=brand_has_model.brand_brand_id 
    INNER JOIN `model` ON model.model_id=brand_has_model.model_model_id WHERE product.id='" . $pid . "'");
...
?>
        ...
        <head>
            ...
            <title><?php echo $product_data["title"]; ?> | Veloxa</title>
